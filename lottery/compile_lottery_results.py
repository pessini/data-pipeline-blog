    ...
    for page in paginator.paginate(Bucket=bucket_name, Prefix=folder_prefix):
        ...
        for obj in page["Contents"]:
            ...
            try:
                ...
                if any(
                    tag["Key"] == "processed" and tag["Value"] == "false"
                    for tag in tags
                ):
                    unprocessed_files.append(key)
            except Exception as e:
                logger.exception(f"Error getting tags for {key}: {e}")
...
def save_to_duckdb(data: dict, game: str, db_path: str):
    ...
    try:
        ...
        winning_numbers_key = (
            "listaDezenas" if "listaDezenas" in data else "dezenasSorteadasOrdemSorteio"
        )
        ...
        logger.error(
            f"Error saving data to DuckDB for game {game}, draw {draw_number}. Error: {e}"
        )
        raise
