...
def fetch_lottery_result(game: str, draw_number: Optional[int] = None) -> dict:
    ...
    try:
        ...
        if response.status_code == 404:
            logger.warning(
                f"Invalid game name: {game}. API returned 404. Skipping this game."
            )
            return None
        elif response.status_code == 500:
            logger.warning(
                f"Invalid draw number: {draw_number}. API returned 500. Skipping this draw."
            )
            return None
        elif response.status_code != 200:
            logger.warning(
                f"Unexpected API response: {response.status_code} - {response.text}. Skipping."
            )
            return None
...
def save_to_minio(data: dict, game: str, draw_number: Optional[int] = None) -> str:
    ...
    try:
        ...
        if existing_file:
            logger.info(
                f"File already exists: s3://{s3_block.bucket_name}/{filename}. Skipping save."
            )
            return None  # Return None if the file already exists
    except Exception as e:
        logger.info(
            f"File does not exist: s3://{s3_block.bucket_name}/{filename}. Proceeding to save."
        )

    # Save data to MinIO
    try:
        json_data = json.dumps(data).encode("utf-8")
        s3_block.write_path(path=filename, content=json_data)
...
def fetch_lottery_results(draw_number: Optional[int] = None):
    ...
    for game in games:
        ...
        if results is None:
            logger.warning(
                f"Skipping save for game: {game}, draw: {draw_number} due to fetch error."
            )
            continue
        ...
        if not results:
            logger.warning(
                f"No results found for game: {game}, draw: {draw_number}. Skipping save."
            )
            continue
        ...
        if filename:
            ...
            tag_file_as_unprocessed(
                s3_client=s3_client, bucket_name=bucket_name, object_key=filename
            )
